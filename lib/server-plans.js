...
export async function getPlanLimits(plan) {
  ...
  if (!dbPlan) {
    ...
    if (!dbPlan.price) {
      const defaultPrices = { free: '$0', starter: '$10 one-time', pro: '$20 /mo' };
      dbPlan.price = defaultPrices[plan];
    }
    await col.updateOne({ id: plan }, { $set: dbPlan }, { upsert: true });
  } else {
    // Backfill missing keys or upgrade legacy defaults (e.g. starter $5 /mo -> $10 one-time, 25 -> 50 scans, features)
    let needsUpdate = false;
    if (plan === 'starter' && (dbPlan.price === '$5 /mo' || !dbPlan.price)) {
      dbPlan.price = '$10 one-time';
      needsUpdate = true;
    }
    if (plan === 'starter' && dbPlan.scansPerMonth === 25) {
      dbPlan.scansPerMonth = 50;
      needsUpdate = true;
    }
    if (plan === 'starter' && (dbPlan.multiPageCrawl === undefined || dbPlan.multiPageCrawl === false)) {
      dbPlan.multiPageCrawl = true;
      needsUpdate = true;
    }
    if (plan === 'starter' && (dbPlan.competitorScan === undefined || dbPlan.competitorScan === false)) {
      dbPlan.competitorScan = true;
      needsUpdate = true;
    }
...
export async function assertFeatureAccess(plan, feature) {
  ...
  if (!hasAccess) {
    ...
    const labels = {
      ...
      multiPageCrawl: 'multi-page site crawling',
...
export async function countScansSince(userId, since) {
  ...
  const [sec, seo, aeo, geo, aso, brand, perf] = await Promise.all([
    ...
    getCollection(COLLECTIONS.ASO_SCANS).then(c => c.countDocuments(filter)),
  ...
  return sec + seo + aeo + geo + aso + brand + perf;
}

 ...
 * @param {string} type - 'global' | 'key' | 'contact'
 * @returns {boolean} True if the request should be blocked
 */
export function isRateLimited(identifier, type = 'global') {
  const limit = type === 'key' ? API_KEY_LIMIT : type === 'contact' ? CONTACT_LIMIT : GLOBAL_LIMIT;
  const windowMs = type === 'key' ? API_KEY_WINDOW : type === 'contact' ? CONTACT_WINDOW : GLOBAL_WINDOW;
  const now = Date.now();

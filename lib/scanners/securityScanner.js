...
import { discoverSubdomains } from './shared/subdomainDiscovery.mjs';
import { runTakeoverAudit } from './shared/takeoverEngine.mjs';
...
export async function runSecurityScan(url) {
  ...
  try {
    ...
    let cspPassed = !!csp;
    let cspDesc = 'Checks for the presence of a CSP header to mitigate XSS attacks.';
    if (csp) {
      ...
      const hasFrameAncestors = lowerCsp.includes('frame-ancestors');

      if (hasUnsafeInline || hasUnsafeEval) {
        cspPassed = false;
        cspDesc = `CSP header is present but allows unsafe directives (${[hasUnsafeInline && "'unsafe-inline'", hasUnsafeEval && "'unsafe-eval'"].filter(Boolean).join(', ')}).`;
      } else if (!hasFrameAncestors) {
        cspDesc = 'CSP header is present and secure against XSS, but lacks frame-ancestors directive for clickjacking defense.';
      } else {
        cspDesc = 'Strict CSP header present without unsafe-inline directives.';
      }
    }
    addFinding(
      ...
      cspDesc,
      cspPassed,
      'Implement a strict CSP restricting sources for scripts, styles, and data without \'unsafe-inline\' or \'unsafe-eval\'.',
      'Add a strict CSP in next.config.js that allows scripts only from trusted origins.',
    ...
    // Subresource Integrity (SRI) Hash Verification (Starter)
    const externalScripts = [];
    const CDN_REGISTRY = /cdnjs\.cloudflare\.com|cdn\.jsdelivr\.net|unpkg\.com|code\.jquery\.com|stackpath\.bootstrapcdn\.com|maxcdn\.bootstrapcdn\.com/i;

    $('script[src], link[rel="stylesheet"][href]').each((_, el) => {
      const src = $(el).attr('src') || $(el).attr('href') || '';
      const integrity = $(el).attr('integrity');
      if (src && CDN_REGISTRY.test(src)) {
        externalScripts.push({ src, hasIntegrity: !!integrity });
      }
    });

    const missingCdnSri = externalScripts.filter(s => !s.hasIntegrity);
    const sriCheckPassed = externalScripts.length === 0 || missingCdnSri.length === 0;

    addFinding(
      'Infrastructure', 'medium', 'Subresource Integrity (SRI) Hashes',
      externalScripts.length === 0
        ? 'No external CDN scripts or stylesheets were detected on the page.'
        : sriCheckPassed
        ? `All ${externalScripts.length} external CDN asset(s) specify valid Subresource Integrity (SRI) cryptographic hashes.`
        : `${missingCdnSri.length} of ${externalScripts.length} external CDN assets are missing integrity="sha256-..." attributes. Compromised CDNs can execute arbitrary script payloads without SRI hashes.`,
      sriCheckPassed,
      'Add integrity="sha384-..." and crossorigin="anonymous" attributes to all external CDN script and stylesheet tags.',
      'How to generate Subresource Integrity (SRI) hashes for CDN script tags.',
      'Subresource Integrity (SRI) enables browsers to verify that resources fetched from CDNs are delivered without unexpected manipulation. If a CDN is compromised, the browser rejects the altered file.',
      'starter'
    );
    ...
    try {
      ...
      if (sitemapRes.ok) {
        ...
        // Match sensitive names as whole path SEGMENTS, not raw substrings.
        // A naive includes('/admin') check false-flags legitimate content URLs
        // like /learn/rules/admin-panel-exposure (an article that merely
        // documents the concept), so compare each decoded path segment exactly.
        const sensitiveSegments = new Set([
          'admin', 'wp-admin', 'administrator', 'phpmyadmin', 'cpanel',
          'internal', 'staging', 'debug', 'config', 'private', 'api',
        ]);
        const urlMatches = sitemapXml.match(/<loc>(.*?)<\/loc>/g) || [];
        for (const match of urlMatches) {
          const sitemapUrl = match.replace(/<\/?loc>/g, '').toLowerCase();
          let pathname;
          try { pathname = new URL(sitemapUrl).pathname; } catch { pathname = sitemapUrl; }
          const segments = pathname.split('/').filter(Boolean);
          if (segments.some(seg => sensitiveSegments.has(seg))) {
    ...
    let subdomainRadarData = null;
    {
      const discoveredAssets = await discoverSubdomains(domain, { maxSubdomains: 40 });
      const subdomains = discoveredAssets.map((a) => a.host).filter((h) => h !== domain);

      if (subdomains.length > 0) {
        // FREE: attack-surface size (count only - teaser)
        addFinding(
          'Attack Surface', 'low', 'Subdomain Exposure',
          `${subdomains.length} subdomain(s) discovered via Certificate Transparency logs and DNS enumeration.`,
          ...
          'Every public subdomain is another way in. Attackers enumerate Certificate Transparency logs to map your infrastructure - forgotten staging, dev, and admin subdomains are prime targets. A smaller, known surface is easier to defend.',
          'free'
        );
        ...
        addFinding(
          ...
          `Discovered subdomains: ${shown.join(', ')}${subdomains.length > shown.length ? ` (+${subdomains.length - shown.length} more)` : ''}.`,
          true,
          ...
          'This is the same passive and active reconnaissance an attacker runs first. Knowing exactly what you expose lets you lock down or remove what should not be internet-facing.',
          'starter'
        );

        // STARTER: Comprehensive Subdomain Takeover & Dangling DNS Audit (40+ Cloud Providers)
        const takeoverAudit = await runTakeoverAudit(discoveredAssets, 8);
        subdomainRadarData = takeoverAudit;

        const vulnHosts = takeoverAudit.assets.filter((a) => a.isVulnerable).map((a) => `${a.host} (${a.provider})`);
        const danglingHosts = takeoverAudit.assets.filter((a) => a.isDangling && !a.isVulnerable).map((a) => `${a.host} → ${a.target}`);

        const hasTakeovers = vulnHosts.length > 0;
        const hasDangling = danglingHosts.length > 0;
        const passed = !hasTakeovers && !hasDangling;

        let description = `Scanned ${takeoverAudit.totalScanned} discovered host(s) across 40+ cloud providers. No dangling DNS or takeover vulnerabilities found.`;
        if (hasTakeovers) {
          description = `CRITICAL: ${vulnHosts.length} vulnerable subdomain takeover(s) detected: ${vulnHosts.join('; ')}.`;
        } else if (hasDangling) {
          description = `HIGH: ${danglingHosts.length} dangling CNAME record(s) pointing to dead/unresolvable targets: ${danglingHosts.join('; ')}.`;
        }

        addFinding(
          'Attack Surface',
          hasTakeovers ? 'critical' : (hasDangling ? 'high' : 'critical'),
          'Subdomain Takeover & Dangling DNS Radar',
          description,
          passed,
          hasTakeovers
            ? 'Immediately claim the orphaned cloud asset (e.g. S3 bucket, Vercel/GitHub Pages domain) or delete the CNAME record from DNS.'
            : (hasDangling ? 'Remove the orphaned CNAME records pointing to dead domains to prevent registration hijacking.' : 'Continue routine audits of DNS records when decommissioning cloud services.'),
          'How do I remediate dangling CNAME records and subdomain takeovers?',
          'A subdomain pointing to an unclaimed third-party cloud service or unresolvable domain can be hijacked by an attacker, allowing them to host phishing pages, steal session cookies (.example.com), and bypass CSP security.',
          'starter'
        );
      }
    }
    ...
    if (emails && emails.length > 0) {
      ...
      // Role-based / functional mailboxes are deliberately published contact
      // points, not personal data — flagging them as PII exposure is a false
      // positive (and contradicts the trust-signal value of a visible contact
      // email). Only a personal-looking address is treated as exposed PII.
      const rolePrefixes = new Set([
        'support', 'info', 'hello', 'contact', 'admin', 'sales', 'help', 'team',
        'noreply', 'no-reply', 'privacy', 'security', 'legal', 'billing',
        'careers', 'press', 'abuse', 'office', 'mail', 'enquiries', 'inquiries',
      ]);
      const realEmails = emails.filter(e => {
        if (genericDomains.some(d => e.includes(d)) || e.includes('yourdomain')) return false;
        const local = e.split('@')[0].toLowerCase();
        return !rolePrefixes.has(local);
      });
    ...
    // \bssn\b (not a bare "ssn" substring): without the word boundaries this
    // matches the "ssn" inside "className", which appears dozens of times in
    // every Next.js/React RSC hydration payload — a guaranteed false positive.
    const hasPii = /(\bssn\b|social security|\b\d{3}-\d{2}-\d{4}\b)/.test(htmlContent);
    ...
    // ADDITIONAL PRO TIER CHECKS
    // ═══════════════════════════════════════════════════════════════════════════
    addFinding('Compliance', 'info', 'OWASP Top 10 Mapping', 'Maps all discovered vulnerabilities to OWASP Top 10 categories.', false, 'Review findings against OWASP A01-A10 classifications.', '', 'Aligning findings with the OWASP Top 10 helps prioritize fixes based on industry-standard risk models.', 'pro');
    addFinding('Compliance', 'info', 'GDPR Compliance Indicators', 'Analyzes tracking, consent, and data protection markers.', hasCookieConsent && hasPrivacyPolicy, 'Ensure explicit opt-in for EU users and valid DPA agreements.', 'GDPR compliance checklist for SaaS applications.', 'GDPR violations can result in fines up to 4% of global revenue. Basic technical indicators show whether compliance frameworks are actively implemented.', 'pro');
    ...
    addFinding('Compliance', 'info', 'SOC 2 Readiness Indicators', 'Checks logging, access control, and encryption markers.', !!hsts, 'Implement robust audit logging and continuous security monitoring.', 'SOC 2 Type II technical requirements for startups.', 'SOC 2 requires demonstrating that security controls are not only implemented but actively monitored and enforced.', 'pro');
  ...
  return {
    ...
    subdomainRadar: subdomainRadarData,

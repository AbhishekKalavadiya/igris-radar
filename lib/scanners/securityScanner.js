...
export async function runSecurityScan(url) {
  ...
  try {
    ...
    if (error.code === 'BOT_CHALLENGE') {
      const err = new Error(
        `${normalizedUrl} is behind a ${error.vendor || 'bot'} challenge that blocked the scanner. ` +
        `Turn off Attack Challenge Mode, or set SCANNER_BYPASS_TOKEN and add a firewall rule that skips the ` +
        `challenge for requests carrying the "x-igris-scan-key" header, then run the scan again.`
      );

...
export async function runSeoScan(url, { plan = 'free' } = {}) {
  ...
  try {
    ...
    // Keyword prominence (Starter) — primary keyword in the first 100 words.
    const primarySource = (h1Count === 1 ? $('h1').first().text() : title) || title;
    const stopWords = new Set(['the', 'a', 'an', 'and', 'or', 'to', 'of', 'for', 'in', 'on', 'with', 'your', 'you', 'is', 'are', 'how', 'best']);
    const primaryKeyword = primarySource
      .toLowerCase()
      .replace(/[^a-z0-9\s]/g, ' ')
      .split(/\s+/)
      .filter(w => w.length > 2 && !stopWords.has(w))[0];
    if (primaryKeyword) {
      const first100 = (proseText || textContent).toLowerCase().split(/\s+/).slice(0, 100).join(' ');
      const inIntro = first100.includes(primaryKeyword);
      addFinding(
        'On-Page SEO', 'medium', 'Keyword Prominence',
        `Primary keyword "${primaryKeyword}" ${inIntro ? 'appears' : 'is missing'} in the first 100 words of body content.`,
        inIntro,
        'Mention your primary keyword early — ideally within the opening paragraph — to reinforce topical relevance.',
        `Rewrite the opening paragraph to naturally include the keyword "${primaryKeyword}" within the first 100 words.`,
        '', 'starter'
      );
    }
    ...
    // Below Agency: teaser placeholders for the AI semantic features (delivered
    // in the AI Insights tab for Agency users) so the Agency tab shows blurred
    // upsell cards to Free/Starter/Pro users.
    if (rank < TIER_RANK.agency) {
      addTeaser('On-Page SEO', 'high', 'AI Search Intent Mapping', 'agency');
      addTeaser('On-Page SEO', 'medium', 'LSI Keyword Gap Analysis', 'agency');
      addTeaser('On-Page SEO', 'medium', 'Semantic Coverage Score', 'agency');
    }

...
export async function runSeoScan(url, { plan = 'free' } = {}) {
  ...
  try {
    ...
    const hasHreflang = hreflangEls.length > 0;
    let hreflangValid = true;
    let invalidHreflangMsg = '';
    ...
    if (hasHreflang) {
      hreflangEls.each((_, el) => {
        ...
        if (!href || !lang) {
          hreflangValid = false;
          invalidHreflangMsg = 'Missing href or hreflang attribute.';
        } else if (!href.startsWith('http')) {
          hreflangValid = false;
          invalidHreflangMsg = `Relative URL found in hreflang ("${href}"). Hreflang must use absolute URLs.`;
        }
      });
    }

    addFinding(
      'Technical SEO', 'low', 'Hreflang Tags & Absolute Alignment',
      !hasHreflang
        ? 'No hreflang tags found. (Optional for single-language sites, required for multi-regional sites).'
        : hreflangValid
        ? `Found ${hreflangEls.length} valid absolute hreflang tag(s) matching internationalization standards.`
        : `Hreflang issue detected: ${invalidHreflangMsg}`,
      !hasHreflang || hreflangValid,
      'Ensure all hreflang tags use absolute URLs starting with https:// and valid language-region codes (e.g. en-US).',
      'Fix hreflang tags to use absolute HTTPS URLs and valid ISO language codes.',
      '', 'starter'
    );
    ...
    // Explicit image dimensions (width & height) to prevent CLS jumps (Free)
    ...
    images.each((_, el) => {
      ...
      if ((!width || !height) && !hasAspectRatio) {
        imagesWithoutDimensions += 1;
      }
    });
    const dimensionCheckPassed = images.length === 0 || imagesWithoutDimensions === 0;
    addFinding(
      'Technical SEO', 'medium', 'Explicit Image Dimensions (CLS Markers)',
      images.length === 0
        ? 'No <img> elements were found on the page.'
        : dimensionCheckPassed
        ? 'All <img> elements specify explicit width/height attributes or CSS aspect-ratio.'
        : `${imagesWithoutDimensions} of ${images.length} <img> elements are missing explicit width/height attributes or aspect-ratio styles. Images without dimensions cause Cumulative Layout Shift (CLS) as they load.`,
      dimensionCheckPassed,
      'Add explicit width and height attributes (or CSS aspect-ratio) to all <img> tags to reserve layout space and eliminate Cumulative Layout Shift (CLS).',
      'Add width and height attributes to these <img> tags in Next.js Image component.',
      '', 'free'
    );
    ...
    if (keyTerms.length > 0) {
      ...
      // LSI Topical Keyword Co-occurrence (Starter)
      const fullText = (proseText || textContent).toLowerCase();
      ...
      for (const term of keyTerms) {
        ...
        termFrequencies[term] = matches;
        totalHits += matches;
      }

      const lsiDensity = totalWords > 0 ? (totalHits / totalWords) * 100 : 0;
      const coveredTermsCount = Object.values(termFrequencies).filter(c => c > 0).length;
      const lsiPassed = totalWords > 0 && coveredTermsCount >= Math.min(2, keyTerms.length) && lsiDensity >= 0.5 && lsiDensity <= 4.0;

      addFinding(
        'On-Page SEO', 'medium', 'LSI Topical Keyword Density',
        totalWords === 0
          ? 'No prose text available to analyze topical keyword density.'
          : `${coveredTermsCount} of ${keyTerms.length} core topic terms appear in the body content (combined LSI density: ${lsiDensity.toFixed(2)}%). Optimal LSI density is 0.5%–3.5%.`,
        lsiPassed,
        'Ensure key topical terms and related LSI synonyms co-occur naturally in body paragraphs (aim for 0.5%–3.5% cumulative density) without keyword stuffing.',
        'Enrich body paragraphs with related LSI terms to improve topical depth.',
        '', 'starter'
      );
    }

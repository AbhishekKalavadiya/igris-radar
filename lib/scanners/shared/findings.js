...
export function createFindingsCollector() {
   ...
   *
   * SEVERITY CONVENTION (read before adding checks): `severity` is the STATIC
   * weight the check carries IF IT FAILS — how bad the problem is — e.g. 'critical'
   * or 'high'. It must NOT depend on the outcome. Do NOT pass
   * `passed ? 'passed' : 'high'`: the `passed` boolean is the ONLY thing that
   * decides pass/fail, and scoring.js weights every check by this value (stored as
   * `originalSeverity`). Folding 'passed' into it collapses a passing check's weight
   * to the 3-point 'passed' fallback while its failing twin keeps full weight, which
   * skews the score ~2× harsh (this was a real bug across asoScanner.js). Use 'info'
   * for always-pass, purely informational checks (weight 0 — score-neutral). The
   * DISPLAYED severity is derived automatically as `passed ? 'passed' : severity`.
   *
   * @param {string} category
   * @param {'critical'|'high'|'medium'|'low'|'info'} severity - Static weight if the check FAILS; never 'passed'
   * @param {string} title
   * @param {string} description
   * @param {boolean} passed - Whether the check passed; this alone determines pass/fail
   * @param {string} [remediation]
   * @param {string} [aiFixPrompt]
   * @param {string} [details]
   * @param {'free'|'starter'|'pro'} [tier='free'] - Which plan tier unlocks this finding
   * @param {boolean} [approximate=false] - If true, this finding uses estimated data
   */
  const addFinding = (category, severity, title, description, passed, remediation = '', aiFixPrompt = '', details = '', tier = 'free', approximate = false) => {
 ...
 * @param {string} userPlan - The user's current plan ('free'|'starter'|'pro')

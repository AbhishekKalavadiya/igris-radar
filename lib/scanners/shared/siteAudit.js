...
export async function runSiteAudit(startUrl, { maxPages = 8, delay = 400 } = {}) {
  ...
  // Index-quality signals (title, meta, canonical, H1, thin content, duplicate
  // titles) only matter for pages search engines are allowed to index. Pages
  // that ship <meta name="robots" content="noindex"> (login, signup, cart,
  // checkout, account) are intentionally excluded from search, so counting
  // their missing meta/thin content against the site is a false positive —
  // Google never sees them. Score those findings against indexable pages only.
  const indexable = pages.filter(p => !p.noindex);
  const idxTotal = indexable.length;
  ...
  addFinding(
    ...
    `${missingMeta} of ${idxTotal} indexable pages are missing a meta description.`,
    missingMeta === 0,
  ...
  const missingTitle = indexable.filter(p => !p.title).length;
  addFinding(
    CATEGORY, 'high', 'Site-Wide Title Tags',
    `${missingTitle} of ${idxTotal} indexable pages are missing a <title> tag.`,
    missingTitle === 0,
  ...
  const noH1 = indexable.filter(p => p.h1Count === 0).length;
  addFinding(
    CATEGORY, 'medium', 'Site-Wide H1 Coverage',
    `${noH1} of ${idxTotal} indexable pages have no H1 heading.`,
    noH1 === 0,
    'Ensure every page has exactly one descriptive H1 heading.',
    '', '', 'free'
  );

  const thin = indexable.filter(p => p.wordCount < 300).length;
  addFinding(
    CATEGORY, 'medium', 'Thin Content Pages',
    `${thin} of ${idxTotal} indexable pages have thin content (< 300 words).`,
    thin === 0,
    'Expand thin pages with more useful, in-depth content or consolidate them.',
    '', '', 'free'
  );

  const noCanonical = indexable.filter(p => !p.hasCanonical).length;
  addFinding(
    CATEGORY, 'low', 'Site-Wide Canonical Tags',
    `${noCanonical} of ${idxTotal} indexable pages have no canonical tag.`,
    noCanonical === 0,

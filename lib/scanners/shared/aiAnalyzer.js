...
export async function runDeepGeoAnalysis(html, url, opts = {}) {
  try {
    ...
    let prompt = `Analyze this HTML content for Generative Engine Optimization (GEO) - how likely are LLMs to trust, cite, and recommend this entity and content. Target URL: ${url}
${localeContext(opts.locale)}
Content:
${contentToAnalyze}
...
Respond in JSON format:
{
  ...
  "citationSimulationDetails": {
    "targetQuery": "The single most likely user query an AI assistant would receive regarding this page's main topic",
    "wouldCite": true,
    "extractedSnippet": "The exact passage you would quote from the page when answering",
    "reasoning": "Detailed role-play explanation of why an LLM (ChatGPT/Claude/Perplexity) would or would not cite this page"
  },
  "knowledgeGraphGapAnalysis": {
    "score": 80,
    "missingProperties": ["foundingDate", "headquarters", "keyPeople"],
    "recommendations": "Detailed advice on missing Organization/Person schema properties needed for Google Knowledge Graph resolution"
  },
  "topicalAuthorityDepthMap": {
    "coveredSubtopics": ["subtopic 1", "subtopic 2"],
    "missingSubtopics": ["essential missing subtopic 1", "missing subtopic 2"],
    "depthAssessment": "Evaluation of what subtopics the page must add to become the definitive source on this subject"
  },
  "competitivePositioning": {
    "trustHierarchyRank": "Tier 1 (Primary Source)",
    "positioningAnalysis": "How the brand ranks against industry competitors in LLM trust hierarchy and specific recommendations to outrank them"
  }
}

Be thorough - this is a paid, in-depth Pro GEO report. For knowledgeGraphGapAnalysis, evaluate entity completeness. For topicalAuthorityDepthMap, list 4-6 specific subtopics this page SHOULD cover to be the definitive source. For competitivePositioning, assess trust hierarchy rank (Tier 1 Primary Source, Tier 2 Secondary Reference, Tier 3 Commodity).`;
    } else {
      prompt += `
}`;
    }

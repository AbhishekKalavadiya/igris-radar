...
// 'flash-lite' is faster and has higher free-tier quota, so it's an excellent
// fallback when the primary flash model is saturated (503 / slow).
...
function buildSecurityStreamPrompt(findings, url, plan) {
  ...
  const fieldSpecs = [
    `{"field":"executiveSummary","value":"A 2-3 sentence high-level summary of the overall security posture."}`,
    `{"field":"topRisks","value":["most dangerous risk 1","risk 2","risk 3"]}`,
  ];
  if (isAgency) {
    fieldSpecs.push(
      `{"field":"threatModeling","value":"A paragraph describing how an attacker could chain these vulnerabilities into a real breach."}`,
      `{"field":"remediationCodeSnippets","value":[{"title":"Example Nginx Fix","code":"server { ... }"}]}`,
      `{"field":"owaspMapping","value":["A05:2021-Security Misconfiguration","A01:2021-Broken Access Control"]}`,
      `{"field":"complianceReadiness","value":{"gdpr":"High/Medium/Low risk","pci":"High/Medium/Low risk"}}`,
    );
  } else {
    fieldSpecs.push(`{"field":"remediationSummary","value":"A concise 2-sentence summary of the required fixes."}`);
  }
...
Emit these fields, in this exact order, filled with real analysis of the findings above:
${fieldSpecs.join('\n')}`;
}
...
async function* streamProviderText(prompt) {
  ...
  if (resolved !== 'gemini') {
    const text = resolved === 'openai' ? await callOpenAI(prompt, keys.openai) : await callAnthropic(prompt, keys.anthropic);
    yield text;
    return;
  }

...
const GEMINI_ATTEMPT_TIMEOUT_MS = 12000; // per single request
const GEMINI_TOTAL_BUDGET_MS = 30000;    // across all retries/fallbacks
...
async function callAI(prompt, { provider } = {}) {
  ...
  // Hard ceiling so a slow/hung provider can never stall the scan request.
  return withTimeout(run, 35000, 'AI analysis');
}

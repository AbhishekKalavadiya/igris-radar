  ...
  for (const modelName of GEMINI_MODELS) {
    for (let attempt = 1; attempt <= GEMINI_MAX_ATTEMPTS; attempt++) {
      try {
        ...
        const overloaded = /\b(503|429|500)\b/.test(msg) || /overload|high demand|unavailable|rate limit|quota|try again/i.test(msg);
        ...
        // Transient overload — brief exponential backoff, then retry the same model.
        if (attempt < GEMINI_MAX_ATTEMPTS) await sleep(500 * attempt);
      }
    }
  }
  throw lastErr;

...
export function classifyScanError(error) {
  ...
  if (code === 'BOT_CHALLENGE') {
    ...
    return {
      ...
      remediation:
        `Allow the scanner through your firewall: temporarily turn off "Under Attack" mode or add a firewall exception that permits traffic from our scanner, then run the scan again.`,
      aiFixPrompt:

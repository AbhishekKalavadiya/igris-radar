...
export async function runAsoScan(url) {
  ...
  const {
    ...
    ratingCountCurrentVersion, recentReviewCount
  } = appData;
  ...
  const titleUtilPct = Math.round((titleLen / maxTitleLen) * 100);
  const titlePassed = titleLen > 0 && titleLen <= maxTitleLen && titleUtilPct >= 70;
  addFinding(
    'metadata',
    titleLen === 0 ? 'critical' : (titleUtilPct < 70 ? 'medium' : 'high'),
    'App Title Character Utilization',
    `Your title is ${titleLen}/${maxTitleLen} characters (${titleUtilPct}% utilization). ${titleUtilPct < 70 ? 'Using under 70% of character allowance wastes keyword indexing space.' : 'Good title character utilization.'}`,
    titlePassed,
    'Utilize at least 70% of available title space (e.g., "BrandName - Feature Keyword") to maximize store search indexing.',
    `Rewrite this app title "${title}" to utilize 25-30 characters with strong ASO keywords.`,
    `Current: ${titleLen}/${maxTitleLen} (${titleUtilPct}%)`,
    TIER
  );
  ...
  const subUtilPct = Math.round((subLen / maxSubLen) * 100);
  const subPassed = subLen > 0 && subLen <= maxSubLen && subUtilPct >= 65;
  addFinding(
    'metadata',
    subLen === 0 ? 'high' : (subUtilPct < 65 ? 'medium' : 'high'),
    'Subtitle / Short Description Utilization',
    `Your short description is ${subLen}/${maxSubLen} characters (${subUtilPct}% utilization). ${subUtilPct < 65 ? 'Adding keywords improves store conversion and search reach.' : 'Good short description length.'}`,
    subPassed,
    'Utilize available short description space to convey your main value proposition and key search terms.',
    `Write a compelling ${maxSubLen}-character short description utilizing 80%+ of available character allowance.`,
    `Current: ${subLen}/${maxSubLen} (${subUtilPct}%)`,
    TIER
  );

  // 3. Long Description Quality & Bullet Formatting
  ...
  const formattingPassed = descWords > 100 && hasBullets;
  addFinding(
    'metadata',
    'medium',
    'Description Bullet Formatting & Scannability',
    hasBullets
      ? 'Long description uses bullet points to structure feature highlights for easy scanning.'
      : 'Description lacks bullet points. Bulleted feature lists increase App Store conversion rates by up to 25%.',
    formattingPassed,
    'Format your description with bullet points (• or ✔) highlighting top 4-6 key features so store visitors can scan quickly.',
    `Format this description into structured bulleted feature sections: ${description.substring(0, 150)}...`,
    hasBullets ? 'Bullets present' : 'Wall of text',
    TIER
  );
  ...
  addFinding(
    'metadata',
    daysSinceUpdate < 180 ? 'medium' : 'high',
    'Update Recency',
  ...
  if (platform === 'ios') {
    ...
    addFinding(
      'technical',
      'info', // Android apps are generally universal, assume pass for basic scan
  ...
  // 12. Developer response presence. Best-effort: neither the iTunes lookup nor the
  // Play scraper exposes reply data as a structured field, so we detect it in the
  // page's embedded data — Apple's shoebox JSON key (require the ':' so it's a real
  // key, not the phrase in prose) or Google Play's "Developer response" label.
  const hasDevResponse = /"developerResponse"\s*:/.test(html) || /Developer\s+response/i.test(html);
  addFinding(
    'reviews',
    'medium',
    'Developer Responses',
  ...
  // 13. Recent review activity — derived from STRUCTURED store data, not a
  // substring match on the current year. The old html.includes(year) matched
  // copyright footers, cache-busting params and script versions, so it passed on
  // nearly every page regardless of real activity. iOS exposes ratings on the
  // current version; the Play scraper exposes a sample of recent review comments.
  const recentReviewSignal = platform === 'ios'
    ? (ratingCountCurrentVersion || 0)
    : (recentReviewCount || 0);
  const hasRecentReviews = recentReviewSignal > 0;
  addFinding(
    'reviews',
    'low',
    ...
    '',
    platform === 'ios'
      ? `${ratingCountCurrentVersion || 0} ratings on the current version`
      : `${recentReviewCount || 0} recent reviews sampled`,
    TIER
  );

  // Category: visuals
  // 14. Screenshot count & caption guidance
  const ssCount = screenshots.length;
  const ssPassed = ssCount >= 5;
  addFinding(
    'visuals',
    'high',
    'Screenshot Count & Caption Overlays',
    `You have ${ssCount} screenshot(s). ${ssCount < 5 ? 'Using at least 5 screenshots with bold feature captions maximizes visual real estate and conversion.' : 'Good screenshot volume.'}`,
    ssPassed,
    'Upload 5+ high-resolution screenshots featuring bold top-caption overlays explaining key benefits.',
    'Suggest 5 screenshot caption overlays for this app concept.',
    `Count: ${ssCount}`,
    TIER
  );
  ...
  if (platform === 'android') {
    ...
    addFinding('visuals', 'info', 'Feature Graphic', 'N/A for iOS.', true, '', '', '', TIER);
  }
  ...
  if (platform === 'android') {
    ...
    addFinding('competition', 'info', 'iOS Download Estimate', 'N/A for Android', true, '', '', '', TIER, true);
  } else {
    ...
    addFinding('competition', 'info', 'Android Install Bracket', 'N/A for iOS', true, '', '', '', TIER, true);
  }

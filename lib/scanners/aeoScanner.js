...
export async function runAeoScan(url, { plan = 'free' } = {}) {
  ...
  try {
    ...
    $('h2, h3').each((_, el) => {
      totalHeadingsChecked += 1;
      const nextP = $(el).next('p');
      if (nextP.length > 0) {
        const text = nextP.text().trim();
        const wordCount = text.split(/\s+/).filter(Boolean).length;
        if (wordCount > 0 && wordCount <= 50) directAnswersCount += 1;
      }
    });
    const directAnswerRatio = totalHeadingsChecked > 0 ? directAnswersCount / totalHeadingsChecked : 0;
    const hasDirectAnswers = totalHeadingsChecked > 0 && directAnswerRatio >= 0.5;
    addFinding(
      'Content Structure', 'high', 'Direct Answer Paragraphs',
      totalHeadingsChecked === 0
        ? 'No H2/H3 headings were found to evaluate for direct answer paragraph structures.'
        : `${directAnswersCount} of ${totalHeadingsChecked} subheadings are immediately followed by a concise (≤50 words) direct answer paragraph. AI engines lift these 40-word answer blocks directly into answer boxes.`,
      hasDirectAnswers,
      'Provide a short, direct answer (≤50 words) immediately after a question heading before expanding into background details.',
      'Write a 40-word direct answer immediately under this subheading.',
      '', 'pro'
    );

    // ─── Pro: Pronoun Ambiguity & Antecedent Resolution ───
    const sectionParas = [];
    $('h2, h3').each((_, el) => {
      const nextP = $(el).next('p').text().trim();
      if (nextP) sectionParas.push(nextP);
    });
    let ambiguousParas = 0;
    const AMBIGUOUS_SUBJECT = /^(it|they|these|this|those)\s+(is|are|was|were|has|have|can|will|should|would|could|provides|helps|allows|means)\b/i;
    for (const p of sectionParas) {
      if (AMBIGUOUS_SUBJECT.test(p)) ambiguousParas += 1;
    }
    const pronounCheckPassed = sectionParas.length === 0 || ambiguousParas === 0;
    addFinding(
      'Content Structure', 'medium', 'Pronoun Ambiguity Antecedent Resolution',
      sectionParas.length === 0
        ? 'No section paragraphs were found to evaluate for pronoun ambiguity.'
        : pronounCheckPassed
        ? 'Section paragraphs open with clear explicit entity nouns rather than ambiguous subject pronouns.'
        : `${ambiguousParas} paragraph(s) open with ambiguous subject pronouns ("It is...", "They provide...") without an explicit entity noun. AI answer engines cut out sections when pronouns are disconnected from entity nouns.`,
      pronounCheckPassed,
      'Replace ambiguous subject pronouns ("It is...", "They offer...") at the start of section paragraphs with explicit brand or entity names so AI engines can extract stand-alone quotes.',
      'Replace opening subject pronouns in these paragraphs with explicit entity names.',
      '', 'pro'
    );
    ...
    // Look for external authority links across technical and academic registers
    let authorityLinksCount = 0;
    const authorityDomains = [];
    const REGISTERS = /wikipedia\.org|arxiv\.org|ncbi\.nlm\.nih\.gov|\.gov|\.edu|w3\.org|ietf\.org|github\.com|developer\.mozilla\.org|docs\.oracle\.com/i;
    $('a[href]').each((_, el) => {
      const href = $(el).attr('href');
      if (href && REGISTERS.test(href)) {
        authorityLinksCount += 1;
        try {
          const host = new URL(href).hostname;
          if (!authorityDomains.includes(host)) authorityDomains.push(host);
        } catch (e) {}
      }
    });
    const hasAuthorityLinks = authorityLinksCount > 0;
    const domainsSummary = authorityDomains.slice(0, 3).join(', ');
    addFinding(
      'Citation-Readiness', 'medium', 'Source Citations & Authority Registers',
      hasAuthorityLinks
        ? `Found ${authorityLinksCount} outbound citation(s) to authoritative technical and academic registers${domainsSummary ? ` (${domainsSummary})` : ''}. External authority citations significantly increase LLM trust.`
        : 'No outbound citations to authoritative technical or academic registers (.edu, .gov, Wikipedia, W3C, IEEE, MDN) were detected.',
      hasAuthorityLinks,
      'Cite authoritative sources (.edu, .gov, Wikipedia, MDN, W3C) inline to increase factual credibility and LLM citation weight.',
      'Add inline source citations to authoritative technical or academic registers.',
      '', 'starter'
    );

 ...
 * gated only for display. Premium AI-powered analysis (Agency) is delivered
...
export async function runAeoScan(url, { plan = 'free' } = {}) {
  ...
  try {
    ...
    addFinding(
      'Crawlability', 'high', 'JavaScript Rendering Dependency',
      jsDependent
    ...
    const h1El = $('h1').first();
    const introP = h1El.length ? h1El.nextAll('p').first() : $('p').first();
    ...
    addFinding(
      'Content Structure', 'medium', 'Definition / Summary Block',
      introWords === 0
    ...
    if (rank < TIER_RANK.agency) {
      addTeaser('Content Structure', 'high', 'Answer Engine Simulation', 'agency');
      addTeaser('Content Structure', 'high', 'Question Coverage Gap Analysis', 'agency');
      addTeaser('Citation-Readiness', 'medium', 'Factual Density Scoring', 'agency');
      addTeaser('Citation-Readiness', 'medium', 'Content Freshness AI Scoring', 'agency');
    }

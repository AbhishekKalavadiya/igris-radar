...
export async function runGeoScan(url, { plan = 'free' } = {}) {
  ...
  try {
    ...
    const orgSchemas = schemaData.schemas.filter(s => {
      ...
      return types.includes('Organization') || types.includes('WebSite') || types.includes('Person');
    });
    if (orgSchemas.length > 0) {
      const DISAMBIG_FIELDS = ['description', 'foundingDate', 'founder', 'url', 'sameAs', 'logo', '@id'];
      let bestCoverage = 0;
      let hasAuthoritySameAs = false;
      ...
      for (const org of orgSchemas) {
        ...
        const sameAsArr = Array.isArray(org.sameAs) ? org.sameAs : (typeof org.sameAs === 'string' ? [org.sameAs] : []);
        if (sameAsArr.some(link => /wikidata\.org|wikipedia\.org|linkedin\.com|crunchbase\.com/i.test(link))) {
          hasAuthoritySameAs = true;
        }
        if (typeof org['@id'] === 'string' && (org['@id'].includes('#') || org['@id'].startsWith('http'))) {
          hasCanonicalId = true;
        }
      }

      const pass = bestCoverage >= 4 && (hasCanonicalId || hasAuthoritySameAs);
      addFinding(
        'Entity Authority', 'high', 'Entity Disambiguation Score',
        `Your Organization/WebSite schema populates ${bestCoverage} of ${DISAMBIG_FIELDS.length} disambiguation fields (description, foundingDate, founder, url, sameAs, logo, @id)${hasAuthoritySameAs ? ', including authority register sameAs links' : ''}${hasCanonicalId ? ' and canonical @id' : ''}. Incomplete entity data causes LLMs to confuse your brand with others.`,
        pass,
        'Fill in description, foundingDate, founder, url, sameAs (Wikidata/LinkedIn), logo, and canonical @id on your Organization schema so engines can distinguish your brand from similarly named ones.',
        'Expand my Organization JSON-LD with description, foundingDate, founder, url, sameAs (Wikidata/LinkedIn), logo, and canonical @id fields.',
        '', 'pro'
      );
    } else {
      addFinding(
        ...
        false,
        'Add Organization JSON-LD with description, foundingDate, founder, url, sameAs (Wikidata/LinkedIn), logo, and canonical @id so engines can disambiguate your brand.',
        'Generate complete Organization JSON-LD with description, foundingDate, founder, url, sameAs (Wikidata/LinkedIn), logo, and canonical @id.',
        '', 'pro'
      );
    }
    ...
    const UNIQUENESS_PATTERN = /\b(we tested|we found|we analyzed|our research|our study|our data|case study|in our experience|we surveyed|according to our|we measured|our experiment|our benchmark|in-house|primary data|original analysis|our findings|dataset|expert interview)\b/i;
    const hasOriginalResearch = UNIQUENESS_PATTERN.test(textContent);
    const hasExpertQuote = $('blockquote, cite, q, [class*="quote" i], [class*="author" i]').length > 0;
    const uniquenessSignals = (hasOriginalResearch ? 1 : 0) + (hasExpertQuote ? 1 : 0);
    addFinding(
      'Topical Authority', 'medium', 'Content Uniqueness Indicators',
      `Detected ${uniquenessSignals} original-content signal(s): first-person research (${hasOriginalResearch ? 'yes' : 'no'}), expert quotes/citations (${hasExpertQuote ? 'yes' : 'no'}). Original content is cited far more than rephrased summaries.`,
      uniquenessSignals >= 1,
    ...
    const clusterNames = Object.entries(segmentCounts).filter(([, cnt]) => cnt >= 2).map(([name]) => `/${name}`).join(', ');
    const hasCluster = topCluster >= 3 && clusterShare >= 0.25;
    addFinding(
      'Topical Authority', 'medium', 'Topical Cluster Detection',
      hasCluster
        ? `Internal links concentrate around coherent topic clusters (${topCluster} links share section paths${clusterNames ? `: ${clusterNames}` : ''}). Pillar-and-cluster architecture is strongly rewarded by LLMs.`
      ...
      hasCluster,
      'Organize internal links into pillar-and-cluster architecture — link related articles to a central pillar page under a shared path (e.g. /learn/*, /features/*) so engines see topical depth.',
    ...
    let inlineCitationCount = 0;
    const containerTypes = new Set();
    $('p, li, td, th, figcaption, blockquote').each((_, el) => {
      const $el = $(el);
      ...
      if (external) {
        inlineCitationCount += 1;
        containerTypes.add(el.tagName.toLowerCase());
      }
    });
    const containersList = Array.from(containerTypes).join(', ');
    addFinding(
      'Factual Density', 'medium', 'Wikipedia-Style Inline Citations',
      `${inlineCitationCount} text block(s)${containersList ? ` (<${containersList}>)` : ''} cite an external source inline (within the same element as the claim). Inline citations dramatically increase LLM trust versus links dumped in a footer.`,
      inlineCitationCount >= 2,
      'Place citations inline, right next to the claim they support (same paragraph, list item, or table cell), the way Wikipedia does — not collected in a footer or reference dump.',
    ...
    const statNearYear = new RegExp(`(\\d+(\\.\\d+)?%|\\$\\d+(\\.\\d+)?[KMBT]?|\\b\\d+x\\b|\\b\\d+\\s*(million|billion|trillion)\\b)[^.]{0,60}\\b(20(1|2)\\d)\\b|\\b(20(1|2)\\d)\\b[^.]{0,60}(\\d+(\\.\\d+)?%|\\$\\d+(\\.\\d+)?[KMBT]?|\\b\\d+x\\b|\\b\\d+\\s*(million|billion|trillion)\\b)`, 'i');
    const hasStat = /\d+(\.\d+)?\s?%|\$\d+([.,]\d+)?[KMBT]?|\b\d+x\b|\b\d+\s*(million|billion)\b/i.test(textContent);
    const hasDatedStats = statNearYear.test(textContent);
    const recentYearMentioned = new RegExp(`\\b(${currentYear}|${currentYear - 1}|${currentYear - 2})\\b`).test(textContent);
    const statFreshnessPassed = !hasStat || hasDatedStats;
    addFinding(
      'Factual Density', 'medium', 'Statistic Freshness',
      !hasStat
        ? 'No numerical statistics or metrics were found on the page, so there is nothing to date-qualify.'
        : hasDatedStats
        ...
        : 'Statistics or quantitative claims were found without nearby year references. Undated stats get deprioritized by engines that favor current data.',
      statFreshnessPassed,
    ...
    const COMPARISON_PATTERN = /\b(vs\.?|versus|compared to|alternative(s)? to|pros and cons|pros & cons|best\s+\w+\s+(for|of|to)|top \d+|comparison|matrix|benchmark)\b/i;
    const hasComparisonText = COMPARISON_PATTERN.test(textContent) || COMPARISON_PATTERN.test($('h1, h2, h3').text());
    const hasComparisonTable = $('table').toArray().some(tbl => {
      const txt = $(tbl).text();
      return COMPARISON_PATTERN.test(txt) || $(tbl).find('th').length >= 3;
    });
    const hasComparison = hasComparisonText || hasComparisonTable;
    addFinding(
      'Factual Density', 'high', 'Comparison & "Best Of" Optimization',
      hasComparison
        ? `Comparison patterns (vs., alternatives, pros/cons, "best of" framing${hasComparisonTable ? ', comparison table matrix' : ''}) were detected — the #1 most-cited content type in AI recommendation queries.`
        : 'No comparison or "best of" framing was detected. Comparison content (vs., alternatives, pros/cons, matrix tables) is the most-cited type in AI recommendation queries.',
      hasComparison,
    ...
    const hasVideoOrAudio = $('iframe[src*="youtube" i], iframe[src*="vimeo" i], video, audio').length > 0;
    const hasInteractive = $('details, summary, [class*="callout" i], [class*="accordion" i], aside').length > 0;
    const formats = {
      text: wordCount > 100,
      lists: hasLists,
      tables: hasTables,
    ...
    addFinding(
      ...
      `This page mixes ${formatCount} of 7 content formats (text, lists, tables, code, captioned images, media embeds, interactive callouts). Pages with 3+ formats are extracted more reliably by AI engines.`,
      formatCount >= 3,
      'Diversify formats — combine prose with lists, a comparison table, code samples, captioned images, or callout boxes so engines have multiple extractable structures.',
      'Suggest which content formats (table, list, code, captioned image, callout) to add to enrich this page.',
      '', 'starter'
    );

    // ─── Starter: Heading as complete thoughts & Search Intent Alignment ───
    const headingTexts = $('h2, h3').toArray().map(el => $(el).text().trim()).filter(Boolean);
    let descriptiveHeadings = 0;
    let questionHeadingsCount = 0;
    const INTENT_HEADING_PATTERN = /^(how|what|why|where|when|which|who|is|can|does|should|best|top|step\s+\d)/i;
    for (const h of headingTexts) {
      const words = h.split(/\s+/).filter(Boolean);
      if (words.length >= 3) descriptiveHeadings += 1;
      if (INTENT_HEADING_PATTERN.test(h) || h.endsWith('?')) questionHeadingsCount += 1;
    }
    ...
    addFinding(
      ...
      headingTexts.length === 0
        ? 'No H2/H3 subheadings were found to evaluate for descriptiveness and search-intent alignment.'
        : `${descriptiveHeadings} of ${headingTexts.length} subheadings are descriptive phrases (3+ words) and ${questionHeadingsCount} match direct search question patterns. Descriptive, question-based headings ("How to Install Node.js") match user prompts far better than terse labels ("Installation").`,
      headingTexts.length > 0 && descriptiveRatio >= 0.6,
      'Write headings as complete, descriptive phrases or questions that mirror how users phrase search prompts ("How to Install Node.js"), not terse labels ("Installation").',
    ...
    const summaryMatch = SUMMARY_PATTERN.test(textContent) ||
      $('h2, h3').toArray().some(el => SUMMARY_PATTERN.test($(el).text()));
    const hasStructuredSummaryList = $('ul, ol').toArray().some(list => {
      const prevText = $(list).prev().text();
      const parentText = $(list).parent().text();
      return SUMMARY_PATTERN.test(prevText) || SUMMARY_PATTERN.test(parentText);
    });
    const hasSummary = summaryMatch;
    addFinding(
      'AI Readability', 'high', 'Structured Summary Detection',
      hasSummary
        ? `A TL;DR, executive summary, or key-takeaways section was detected${hasStructuredSummaryList ? ' with a structured bullet list' : ''} — LLMs preferentially extract these as the primary answer snippet.`
      ...
      hasSummary,
      'Add a clearly labelled "TL;DR" or "Key Takeaways" section with bullet points near the top — the block LLMs extract first as a direct answer.',

 ...
 * gated only for display. Premium AI-powered analysis (Pro/Agency) is delivered
...
export async function runGeoScan(url, { plan = 'free' } = {}) {
  ...
  try {
    ...
    const hasEntityId = html.includes('"@id"');
    addFinding(
    ...
    const hasContactPoint = html.includes('ContactPoint') || hasSchemaType('ContactPoint');
    ...
    const hasAddress = hasSchemaType('PostalAddress') || html.includes('PostalAddress') || $('address').length > 0;
    ...
    // Agency users. Show blurred upsell cards to everyone below Agency.
    if (rank < TIER_RANK.agency) {
      addTeaser('Factual Density', 'high', 'Citation Simulation', 'agency');
      addTeaser('Entity Authority', 'high', 'Knowledge Graph Gap Analysis', 'agency');
      addTeaser('Topical Authority', 'high', 'Topical Authority Depth Map', 'agency');
      addTeaser('Entity Authority', 'medium', 'Competitive Positioning Report', 'agency');
    }

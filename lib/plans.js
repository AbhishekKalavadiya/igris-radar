...
const FEATURE_MIN_PLAN = {
  ...
  multiPageCrawl: PLANS.STARTER,
  competitorScan: PLANS.STARTER,
  monitoring:     PLANS.PRO,
  whiteLabel:     PLANS.PRO,
  topologyGraph:  PLANS.PRO,
};
...
export function getUpgradeMessage(currentPlan, reason, planLimits) {
  ...
  const messages = {
    ...
    whiteLabel: {
      title: 'White-label PDF export is a Pro feature',
      description: `Upgrade to ${targetLabel} to download clean, unbranded PDF reports for your clients.`,
    },
    topologyGraph: {
      title: 'Interactive Topology Mapping is a Pro feature',
      description: `Upgrade to ${targetLabel} to unlock OSINT visual graph topology mapping for all attack surface classifiers.`,
    },
  };

...
function validateEnv() {
  ...
  if (missing.length > 0) {
    const msg = `[Igris Radar] Missing required environment variables: ${missing.join(', ')}\nCopy .env.example → .env.local and fill in the values.`
  ...
  if (process.env.NODE_ENV === 'production') {
    if (!secret || secret === DEFAULTS.SESSION_SECRET) {
      throw new Error('[Igris Radar] SESSION_SECRET must be set to a strong random value in production.')
    }
    if (secret.length < 32) {
      throw new Error('[Igris Radar] SESSION_SECRET must be at least 32 characters in production.')
    }
    if (new Set(secret).size < 8) {
      throw new Error('[Igris Radar] SESSION_SECRET has too little entropy — use a random 32+ char string (e.g. `openssl rand -base64 48`).')
    }

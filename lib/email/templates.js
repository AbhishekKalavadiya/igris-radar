...
/**
 * Escapes HTML so user-supplied values (names, URLs, contact messages) can't
 * inject markup or scripts into the rendered email. Applied to every dynamic
 * value interpolated into email HTML.
 */
function esc(value) {
  return String(value ?? '')
    .replace(/&/g, '&amp;')
...
const row = (label, value) =>
  `<p style="margin:6px 0;font-size:14px;"><span style="color:#8b95a1;">${esc(label)}:</span> <strong style="color:#ffffff;">${esc(value)}</strong></p>`

/** Scan-complete alert. */
export function scanAlertEmail({ name, scanType, url, score, findingsCount, criticalCount }) {
  const title = `${esc(scanType)} scan complete — ${esc(score)}/100`
  const html = layout(title, `
    <p style="font-size:14px;">Hi ${esc(name || 'there')}, your scan has finished.</p>
    ${row('Target', url)}
...
export function loginAlertEmail({ name, ip, time }) {
  ...
  const html = layout(title, `
    <p style="font-size:14px;">Hi ${esc(name || 'there')}, a new sign-in to your Igris Radar account was detected.</p>
...
export function passwordResetEmail({ name, resetUrl }) {
  ...
  const html = layout(title, `
    <p style="font-size:14px;">Hi ${esc(name || 'there')}, we received a request to reset your Igris Radar password.</p>
    <p style="margin:20px 0;">
      <a href="${esc(resetUrl)}" style="display:inline-block;background:${BRAND_TEAL};color:#0c1116;font-weight:bold;font-size:14px;padding:12px 24px;border-radius:6px;text-decoration:none;">Reset password</a>
    </p>
    <p style="font-size:13px;color:#8b95a1;">This link expires in 30 minutes and can only be used once. If you didn't request this, you can safely ignore this email — your password will not be changed.</p>
    <p style="font-size:12px;color:#6b7580;word-break:break-all;">${esc(resetUrl)}</p>
  `)
  return { subject: '[Igris Radar] Reset your password', html, text: `Reset your password: ${resetUrl} (expires in 30 minutes)` }
}
...
export function passwordChangedEmail({ name, time }) {
  ...
  const html = layout(title, `
    <p style="font-size:14px;">Hi ${esc(name || 'there')}, your Igris Radar password was changed at ${esc(time)}.</p>
...
export function monitoringAlertEmail({ name, scanType, url, score, threshold, frequency }) {
  const title = `Score alert: ${esc(url)} dropped to ${esc(score)}/100`
  const html = layout(title, `
    <p style="font-size:14px;">Hi ${esc(name || 'there')}, your ${esc(frequency)} ${esc(scanType.toUpperCase())} monitoring detected a score below your alert threshold.</p>
    ${row('Site', url)}
...
export function weeklyDigestEmail({ name, periodLabel, totals, sites }) {
  ...
  const html = layout(`Your weekly SEO &amp; visibility digest`, `
    <p style="font-size:14px;">Hi ${esc(name || 'there')}, here is your summary for ${esc(periodLabel)}.</p>
...
export function contactFormEmail({ firstName, lastName, email, message, time }) {
  ...
  const html = layout(title, `
    ...
    ${row('Email', email)}
    ${row('Time', time)}
    <div style="margin-top:16px;padding:16px;background:#1d2530;border-radius:6px;font-size:14px;color:#d7dde3;white-space:pre-wrap;">${esc(message)}</div>
  `)
